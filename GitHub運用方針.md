...
| GitHub CLI | `gh` は未導入 |
| ローカルリポジトリ | 未初期化 |
| `.git` | 空フォルダーが存在するが、有効なリポジトリではない |
| Gitユーザー名・メール | リポジトリ内だけ `you3-dev` とGitHubの非公開用メールを設定予定 |
| GitHub接続 | `you3-dev/fuiki-ijuroku` を作成済み。ローカル接続前 |
...
GitHub側に `you3-dev/fuiki-ijuroku` が作成された。ローカルリポジトリを初期化し、`.gitignore`、`README.md`、最初のコミットを作成して `origin` へpushする。
